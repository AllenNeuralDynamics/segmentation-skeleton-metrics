...
from seg_eval.graph_routines import *
from seg_eval.seg_metrics import *
from seg_eval.utils import *
...
class MergeMetric(SegmentationMetrics):
    ...
    def __init__(self, shape,
        ...
        if target_volume == None:
            target_volume = super().init_volume(path_to_target_volume, target_graphs_dir)

        if pred_graphs == None:
            pred_graphs = super().init_graphs(pred_graphs_dir, path_to_pred_volume)
        ...
        for graph in self.graphs:
            ...
            while len(dfs_edges) > 0:
                ...
                if super().check_simple_mistake(val_i, val_j):
                    ...
                    dsf_edges, cnt1 = self.explore_merge(graph, dfs_edges, i, val_i)
                    dsf_edges, cnt2 = self.explore_merge(graph, dfs_edges, j, val_j)
    ...
    def process_complex_mistake(self, graph, dfs_edges, edge):
        ...
        while len(queue) > 0:
            ...
            if super().check_simple_mistake(val, val_j):
                dsf_edges, cnt1 = self.explore_merge(graph, dfs_edges, root, val)
                dsf_edges, cnt2 = self.explore_merge(graph, dfs_edges, j, val_j)
                self.merge_edge_cnt += min(cnt1, cnt2)
                self.merge_cnt += 1
